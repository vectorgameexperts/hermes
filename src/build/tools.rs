    ...
    fn format_version_output(&self, text: &str) -> Result<String> {
        ...
        let formatted_version = match self {
            Application::Sass => text
                ...
                .next()
                .with_context(|| {
                    format!("missing or malformed version output: {}", text)
                })?
                .to_owned(),
            ...
            Application::WasmOpt => format!(
                "version_{}",
                text.split(' ').nth(2).with_context(|| format!(
                    "missing or malformed version output: {}",
                    text
                ))?
            ),
        };
...
static GLOBAL_APP_CACHE: Lazy<Mutex<AppCache>> =
    Lazy::new(|| Mutex::new(AppCache::new()));
...
async fn find_system(
    ...
    let result = || async {
        ...
        let output =
            Command::new(&path).arg(app.version_test()).output().await?;
        ensure!(
...
mod archive {
    ...
    impl Archive {
        pub fn new_tar_gz(file: File) -> Self {
            Self::TarGz(Box::new(TarArchive::new(GzDecoder::new(
                BufReader::new(file),
            ))))
        }
        ...
        pub fn extract_file(
            ...
            match self {
                Self::TarGz(archive) => {
                    let mut tar_file = find_tar_entry(archive, file)?
                        .context("file not found in archive")?;
                    let mut out_file =
                        extract_file(&mut tar_file, file, target)?;
                ...
                Self::Zip(archive) => {
                    let zip_index = find_zip_entry(archive, file)?
                        .context("file not found in archive")?;
                    let mut zip_file = archive.by_index(zip_index)?;
                    let mut out_file =
                        extract_file(&mut zip_file, file, target)?;
                ...
                Self::None(in_file) => {
                    ...
                    if let Err(e) = &create_dir_result {
                        if e.kind() != std::io::ErrorKind::AlreadyExists {
                            create_dir_result
                                .context("failed to open file for")?;
                        }
                    }
                    ...
                    let mut out_file = File::create(out_file_path)
                        .context("failed to open binary to copy")?;
                    {
...
mod tests {
    ...
    async fn download_and_install_binaries() -> Result<()> {
        let dir =
            tempfile::tempdir().context("error creating temporary dir")?;
